...
class ViewController: UIViewController, UISearchBarDelegate , UITableViewDataSource {
  ...
  override func viewDidLoad() {
    ...
    // タイトルの設定
    refreshControl.attributedTitle = NSAttributedString(string: "引っ張ると更新できるよ！")
    // 引っ張ったタイミングでコールするメソッドを定義
    refreshControl.addTarget(self, action: #selector(reloadSearch), for: .valueChanged)
  ...
  func searchOkashi(keyword : String) {
    ...
    // URLオブジェクトの生成
    let url = URL(string:
      "http://www.sysbird.jp/toriko/api/?apikey=guest&format=json&keyword=\(keyword_encode!)&max=10&order=r")
    ...
    let task = session.dataTask(with: req, completionHandler: {
      ...
      do {
        ...
        print ("----------------")
        print (self.okashiList.first ?? "データなし")
